  ...
  return (
    <div className={cn('min-h-dvh bg-ink text-paper safe-pt safe-pb', 'flex flex-col')}>
      <header className="flex items-center justify-between px-4 pb-2">
        ...
        <p className="font-mono text-[10px] tracking-[0.35em] text-paper/45">TECHNICAL MANUAL</p>
        <div className="w-11" />
      </header>
        ...
        {info ? (
          ...
          >
            <div className="mb-8 border-b border-paper/15 pb-6">
              <p className="font-mono text-[10px] tracking-[0.35em] text-paper/45">
                EXERCISE {index >= 0 ? ` / ${String(index + 1).padStart(3, '0')}` : ''}
              </p>
              <h1 className="mt-2 text-[clamp(2.25rem,9vw,3.5rem)] font-black uppercase leading-[0.9] tracking-tighter">
                {info.name}
              </h1>
              <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[11px] tracking-[0.25em] text-paper/60">
                <span>{info.muscles.join(' · ')}</span>
                {info.category && <span className="text-paper/40">{info.category.toUpperCase()}</span>}
              </div>
            </div>
            ...
            {info.commonMistakes.length > 0 && (
              <section className="mt-8 border-t border-paper/15 pt-6">
                <h2 className="mb-4 font-mono text-[11px] tracking-[0.35em] text-soviet">COMMON ERRORS</h2>
            ...
            {info.safety && info.safety.length > 0 && (
              <section className="mt-8 border-t border-paper/15 pt-6">
                <h2 className="mb-4 font-mono text-[11px] tracking-[0.35em] text-paper/60">SAFETY</h2>
                <ul className="space-y-3">
                  {info.safety.map((s, i) => (
                    <li key={i} className="flex items-start gap-3 text-paper/80">
                      <span className="font-mono text-paper/40">{String(i + 1).padStart(2, '0')}</span>
                      {s}
                    </li>
                  ))}
                </ul>
            ...
            {(info.easier?.length || info.harder?.length) && (
              <section className="mt-8 border-t border-paper/15 pt-6">
                <h2 className="mb-4 font-mono text-[11px] tracking-[0.35em] text-paper/60">PROGRESSION</h2>
                <div className="grid gap-4 sm:grid-cols-2">
                  {info.easier && info.easier.length > 0 && (
                    <div className="border border-paper/15 p-4">
                      <p className="mb-2 font-mono text-[10px] tracking-[0.25em] text-paper/45">EASIER</p>
                  ...
                  {info.harder && info.harder.length > 0 && (
                    <div className="border border-paper/15 p-4">
                      <p className="mb-2 font-mono text-[10px] tracking-[0.25em] text-paper/45">HARDER</p>
                      <ul className="list-disc space-y-1 pl-4 text-sm text-paper/80">
                        {info.harder.map((e, i) => (
                          <li key={i}>{e}</li>
                        ))}
            ...
            {info.breathing && (
              <section className="mt-8 border-t border-paper/15 pt-6">
                <h2 className="mb-2 font-mono text-[11px] tracking-[0.35em] text-paper/60">BREATHING</h2>
          ...
          <motion.div
            ...
            className="pt-10"
          >
            <p className="font-mono text-[10px] tracking-[0.35em] text-paper/45">TECHNICAL NOTE</p>
...
function StepSection({ label, steps }: { label: string; steps: string[] }) {
  ...
  return (
    <section className="mt-8 border-t border-paper/15 pt-6">
      <h2 className="mb-4 font-mono text-[11px] tracking-[0.35em] text-paper/60">{label}</h2>
      <ol className="space-y-4">
        {steps.map((step, i) => (
          <li key={i} className="flex items-start gap-4">
            <span className="w-6 shrink-0 font-mono text-xs tabular text-paper/40">{String(i + 1).padStart(2, '0')}</span>
