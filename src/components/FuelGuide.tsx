  ...
  return (
    <motion.article variants={list} initial="initial" animate="animate" className="flex flex-col gap-6">
      ...
      <motion.header variants={item} className="relative border-2 border-paper/80 p-5">
        <span className="absolute -top-2.5 left-4 bg-ink px-2 font-mono text-[10px] tracking-[0.35em] text-soviet">
          DIRECTIVE 03 · NUTRITION
        </span>
        <span className="absolute right-4 top-3 -rotate-6 border-2 border-soviet px-2 py-0.5 font-mono text-[10px] font-bold tracking-[0.3em] text-soviet">
          DECLASSIFIED
        </span>
        <p className="mt-3 font-mono text-[10px] tracking-[0.3em] text-paper/45">ДИРЕКТИВА · ПИТАНИЕ</p>
        <h2 className="mt-2 text-[clamp(1.75rem,8vw,2.5rem)] font-black uppercase leading-[0.9] tracking-tighter">
          The Shmondenko
          <br />
          Meat-Free Protocol
        </h2>
        <div className="mt-4 flex items-end justify-between border-t border-paper/20 pt-3">
          ...
          <p className="max-w-[9rem] text-right font-mono text-[9px] leading-relaxed tracking-[0.15em] text-paper/40">
      ...
      <Clause variants={item} index="§2" title="The Blender Battle Shake">
        <div className="border border-paper/15">
          <p className="border-b border-paper/15 px-4 py-3 font-mono text-[10px] tracking-[0.25em] text-paper/45">
          ...
          <p className="border-t border-paper/15 px-4 py-3 font-mono text-[10px] tracking-[0.2em] text-paper/60">
      ...
      <motion.footer variants={item} className="flex items-center justify-between border-t border-paper/15 pt-4">
        <p className="font-mono text-[9px] tracking-[0.3em] text-paper/35">FOR TRAINING PERSONNEL ONLY</p>
        <p className="font-mono text-[9px] tracking-[0.3em] text-paper/35">ФОРМА № 3 · СССР</p>
      </motion.footer>
    </motion.article>
  )
}
...
function Principle({ n, head, children }: { n: string; head: string; children: ReactNode }) {
  return (
    <li className="flex gap-4 py-3">
      <span className="w-8 shrink-0 pt-0.5 font-mono text-xs tabular text-paper/40">{n}</span>
      <div>
...
function Ingredient({ qty, name }: { qty: string; name: string }) {
  return (
    <li className="flex flex-col gap-0.5 px-4 py-3">
      <span className="font-mono text-[10px] font-normal tabular tracking-[0.2em] text-paper/45">{qty.toUpperCase()}</span>
...
function Meal({ time, name, children }: { time: string; name: string; children: ReactNode }) {
  return (
    <li className="py-3">
      <p className="font-mono text-[10px] tracking-[0.3em] text-paper/45">{time}</p>
