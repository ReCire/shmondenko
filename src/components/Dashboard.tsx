...
export function Dashboard() {
  ...
  const currentPhase = useMemo(() => computeCurrentPhase(logs), [logs])
  const last7 = useMemo(() => {
  ...
  const list = tab === 'stock' ? STOCK_WORKOUTS : customWorkouts
  ...
  return (
    <div className="relative mx-auto flex min-h-dvh w-full max-w-lg flex-col safe-pt safe-pb">
      ...
      <header className="flex items-start justify-between px-6 pb-6">
        <div>
          <p className="font-mono text-[11px] tracking-[0.35em] text-paper/45">SHMONDENKO · SOVIET PERIODISATION</p>
          <h1 className="mt-1 text-4xl font-black uppercase leading-[0.9] tracking-tighter">
            Shmondenko
            <span className="text-soviet">.</span>
          </h1>
          <p className="mt-1 font-mono text-[11px] tracking-[0.25em] text-paper/55">
        ...
        <button
          ...
          className="mt-1 flex h-10 w-10 items-center justify-center border border-paper/15 text-paper/60 active:bg-paper/10"
        >
      ...
      <LayoutGroup>
        <nav className="mx-6 flex border-b border-paper/15">
          {(
            [
              ['stock', `STOCK · ${currentPhase.toUpperCase()}`],
          ...
          {list.length === 0 ? (
            ...
            list.map((w, i) => (
              <WorkoutCard
                ...
                workout={w}
                index={i}
                recommended={tab === 'stock' && w.phase === currentPhase}
